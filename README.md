...
1. 캐릭터 소켓 설정
무기는 캐릭터에 트랜스폼으로 배치하는 것이 아니라 메시에 착용해야 캐릭터 애니메이션에 따라 무기가 움직임<br>
언리얼 엔진은 캐릭터에 무기+액세서리를 부착하는 용도로 소켓이라는 시스템을 제공

소켓 우클릭 -> 프리뷰 에셋 추가 -> 무기 이름 검색해서 부착<br>
패키지의 무기 에셋은 스켈레탈 메시, 스켈레탈 메시 컴포넌트를 캐릭터 메시에 부착<br>
SetupAttachment 함수에 소켓 이름을 인자로 넘기면 소켓 위치를 기준으로 트랜스폼이 자동으로 설정

2. 무기 액터의 제작
액터에 고정으로 무기를 장착하지 않고 필요에 따라 무기를 바꾸게 하려면 무기를 액터로 분리해 만드는 것이 좋음<br>
캐릭터가 드는 무기는 충돌을 발생시키지 않고 액세서리로만 -> 무기 액터의 루트 컴포넌트인 스켈레탈 메시 컴포넌트의 충돌 설정을 NoCollision으로 설정

월드에서 새롭게 액터를 생성하는 명령은 SpawnActor, 액터는 월드에 존재하는 물체이므로 월드의 명령이기 때문<br>
GetWorld 함수로 월드의 포인터를 가져와서 해당 함수를 실행, SpawnActor의 인자에는 생성할 액터의 클래스와 액터가 앞으로 생성할 위치/회전을 가져온다

3. 아이템 상자의 제작
플레이어에게 무기를 공급해줄 아이템 상자를 제작<br>
아이템 박스는 콜리전 박스와 스태틱메시로 나뉨
- 콜리전 박스: 플레이어를 감지, 루트 컴포넌트 (박스 콜리전 컴포넌트)
- 스태틱메시: 아이템 상자를 시각화, 박스 콜리전 컴포넌트의 자식 컴포넌트

폰이 아이템을 획득하도록 아이템 상자에 오브젝트 채널 추가<br>
- ItemBox 프리셋(신규생성): 콜리전 켜짐 -> Query Only, ABCharacter에만 겹침 적용, 나머지 전부 무시<br>
- ABCharacter 프리셋: ItemBox 무시 -> 겹침으로 설정

- OnComponentBeginOverlap 델리게이트: 박스 컴포넌트에 Overlap 이벤트를 처리할 수 있음, 멀티캐스트 다이나믹 델리게이트

4. 아이템의 습득
아이템 상자를 통과하면 빈손의 플레이어에게 아이템을 쥐어주는 기능 구현<br>
1. 배치한 아이템 상자에 클래스 정보를 저장할 속성을 추가
2. 이 값을 기반으로 플레이어가 아이템 상자의 영역에 들어왔을 때 아이템을 생성한다

언리얼 엔진은 특정 클래스와 상속받은 클래스들로 목록을 한정하도록 TSubclassof라는 키워드를 제공 -> 목록에서 아이템 상자와 선언한 클래스 목록만 볼 수 있음

상자에 이펙트를 부착해 아이템을 습득하면 이펙트를 재생, 재생하면 상자가 사라지는 기능
1. 상자 액터에 파티클 컴포넌트를 추가
2. 해당 이펙스 애샛의 레퍼런스를 복사하고 파티클 컴포넌트의 템플릿으로 이를 지정
3. 멤버 함수를 하나 추가하고 파티클 컴포넌트 시스템에서 제공하는 OnSystemFinished 델리게이트에 이를 연결해 이펙트 재생이 종료되면 아이템 상자가 제거되도록<br>
OnSystemFinished는 다이나믹 델리게이트 -> 바인딩할 대상 멤버 삼수에 UFUNCTION 매크로 필요

아이템 상자에서 다른 무기 생성
1. 블루프린트 클래스 생성 -> ABWeapon 상속
3. 스켈레탈 메시 컴포넌트의 스켈레탈 메시에서 다른 걸로 변경